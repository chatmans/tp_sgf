...
int main(int argc, char **argv) {
  ...
  printf("Test écriture N°1 : écriture de %d caractères\n", (int) (10 * strlen(mess)));
  ...
  printf("\nNombre d'E/S physiques : %d (%d lectures + %d écritures)\n",physical_io_count, physical_read_count,physical_write_count);
  printf("\n***********************************************************\n");
  printf("Test écriture N°3 : Écriture de 526 caractères dans le fichier test012.txt\n");
  ...
  if (file_r == NULL) {
    ...
    while ((res >= 0) && (nberreur < 5)) {
      if (res != ('A' + (nbcar % 10))) {
	printf("[FAILED] Erreur de relecture caractère N° %d : '%c/%02X' attendu mais '%c/%02X' lu\n",
	       nbcar,
	       (char) ('A' + (nbcar % 10)),
